...
class CustomerDAO extends UserDAO {
  ...
  public async getByEmail(email: string): Promise<Fetch> {
    const query =
      'SELECT u.*, c.client_id FROM client c INNER JOIN user u ON u.id = c.id WHERE u.email = ?';
    const values = [email];
  ...
  public getById = async (id: string): Promise<Fetch> => {
    const query =
      'SELECT u.*, c.client_id FROM customer c INNER JOIN user u ON u.id = c.id WHERE u.id = ?';
    const values = [id];
